...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        while (head != null) {
            ...
            if (head.getKey().equals(key)) { // duplicate key found
        ...
        ExternalChainingMapEntry<K, V> newEntry = new ExternalChainingMapEntry<>(key, value); // Create a new entry
        newEntry.setNext(table[index]);
        table[index] = newEntry;
        size++;
        return null;

        /* table[index] = new ExternalChainingMapEntry<>(key, value, table[index]); // table[index] is the "next" pointer
        size++;
        return null;
        */
    ...
    public V remove(K key) {
        ...
        if (head.getKey().equals(key)) { // if front of the list
        ...
        while (head.getNext() != null) {
            ...
            if (head.getNext().getKey().equals(key)) { // searching

                V value = head.getNext().getValue();
    ...
    public Set<K> keySet() {
        ...
        for (ExternalChainingMapEntry<K, V> head : table) {
    ...
    public void resizeBackingTable(int length) {
        ...
        if (length < size) {
            throw new IllegalArgumentException("The length is less than the number of items in the map.");
        }
        ...
        for (ExternalChainingMapEntry<K, V> head : table) {
            ...
            while (head != null) {
                ...
                int index = Math.abs(head.getKey().hashCode() % temp.length); // LENGTH = 7 for example

                ExternalChainingMapEntry<K, V> newEntry = new ExternalChainingMapEntry<>(head.getKey(), head.getValue());
                newEntry.setNext(temp[index]); // Set the next pointer of the new entry to the current head
                temp[index] = newEntry; // Update the head to point to the new entry
                head = head.getNext();
                counter++;

                /* temp[index] = new ExternalChainingMapEntry<>(head.getKey(), head.getValue(), temp[index]); // the "next" pointer
                head = head.getNext();
                counter++;
